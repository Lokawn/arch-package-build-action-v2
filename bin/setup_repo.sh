create_checksum() {
    echo -e "::group::${GREEN_COLOR}Generating and Signing SHA512SUMS.${UNSET_COLOR}"
        cd "/github/workspace/pkgdir"

        sha512sum -- *.zst | tee "sha512sums" \
            | xargs -d "\n" \
                printf "${BLUE_COLOR}%s${UNSET_COLOR}\n"
...
create_repo() {
    echo -e "::group::${GREEN_COLOR}Generating Arch Repo files.${UNSET_COLOR}"
