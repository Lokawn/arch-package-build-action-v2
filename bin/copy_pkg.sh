...
[[ `echo $?` -ne 0 ]] &&\
    echo -e "\e[0;31mFailed to make 'logdir', 'pkgdir' - aborting.\e[0m" &&\
    exit 1
...
copy_pkg_files() {
    ...
    do
        ...
        echo -e "\e[0;33mSignature for ${app_name} not found, package file removed.\e[0m" && \
...
seg_pkg_files() {
    ...
    if compgen -G ${pkg}-[a*-z]*[0-9]*.zst &> $DEBUG_OFF; then
        find "$GITHUB_WORKSPACE/pkgdir/" -name "${pkg}-[a*-z]*[0-9]*" -delete
        echo -e "\e[0;33m$pkg to be updated.\e[0m"
    else
